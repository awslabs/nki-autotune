...
- [ ] **Step 2 (controller): pick the winner.** The single best-MEASURED sequence that compiles+runs. Honest comparison points: the prior 83.4% (non-rfactor) and the 90.92% hand kernel (`KERNEL_SOURCE` in `examples/profile_matmul_lhsT_rhs_hand.py`). State plainly where the best traced kernel lands between them; if it does NOT beat 83.4%, say so and keep 83.4% as the winner.
