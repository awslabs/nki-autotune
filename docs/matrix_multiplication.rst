...
``M`` dimension; 4 tiles or 2048 numbers in the ``N`` dimension; and 4 tiles or
512 numbers in the ``K`` dimension), this computation has an arithmetic
intensity of 1024 Flops/Byte (2048*2048*512/(2048*512 + 2048*512)). This is
...
``lhsT_tiles`` requires 2MB and ``rhs_tiles`` requires 2MB SBUF memory. The
``result_tiles`` requires ``8 * NUM_BLOCK_M`` MB SBUF memory, where
``NUM_BLOCK_M`` is ``M // 2048``. Thus, as long as ``M <= 4096``, the required
SBUF memory is under the 24 MB budget (2 + 2 + 8 * (4096 // 2048) == 20 MB).
...
Optimization 4: Autotuning the Blocking Parameters
--------------------------------------------------

In the hand-optimized NKI kernel, blocking decisions ``TILES_IN_BLOCK_*`` are
manually chosen.  NKI supports autotuning to automatically find the best
configuration for a specific kernel.  We can use a decorator
``@nki.autotune(configs)`` to enable autotuning.

::

   @nki.autotune(configs= [
      {"TILES_IN_BLOCK_M": 16, "TILES_IN_BLOCK_N": 4, "TILES_IN_BLOCK_K": 4},
      {"TILES_IN_BLOCK_M": 16, "TILES_IN_BLOCK_N": 2, "TILES_IN_BLOCK_K": 8},
   ])
   def nki_matmul_fully_optimized_(lhsT, rhs, result,
                                 # Meta-parameters
   ...
   nki_matmul_fully_optimized_(lhsT, rhs, output)


::

   Progress: 1/2 configurations tested
   Progress: 2/2 configurations tested
   The best latency is 6963 us for config {'TILES_IN_BLOCK_M': 16, 'TILES_IN_BLOCK_N': 2, 'TILES_IN_BLOCK_K': 8}

The experiment shows that the original chosen configuration is not the best
performing one for the matrix multiplication size ``[4096,8192] @ [8192,8192]``,
so we can now use the tuned best configuration for future computation.
...
+-----------------------+---------------+----------------------------------------------+
| Optimization 3        | 7.38          | 80.90                                        |
+-----------------------+---------------+----------------------------------------------+
| Optimization 4        | 6.97          | 85.24                                        |
+-----------------------+---------------+----------------------------------------------+
