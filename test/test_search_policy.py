...
SEARCH_TARGETS = tuple(pytest.param(name, id=name) for name in WORKLOADS)
...
def test_search_policy_reaches_within_tolerance_of_best_historical_latency(
    workload_name: str, policy: Policy, trn2_hosts: tuple[str, ...], tmp_path: Path
) -> None:
    """Each search policy reaches within 3% of the best historical latency."""
    workload = WORKLOADS[workload_name]
    workload_index = tuple(WORKLOADS).index(workload_name)
