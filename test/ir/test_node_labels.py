...
def test_buffer_list_len_default_unchanged():
    """list_len defaults to 1; physical_shape and label are byte-identical to today."""
    from nkigym.ir.tree import Buffer
...
def test_per_tile_physical_shape_splits_middle_dim():
    """list_len>1 divides the tile (middle) dim; partition + free unchanged."""
    from nkigym.ir.tree import Buffer
...
def test_per_tile_physical_shape_identity_when_one():
    """list_len==1: per_tile_physical_shape == physical_shape (the packed buffer)."""
    from nkigym.ir.tree import Buffer
...
def test_buffer_label_shows_list_form_when_list_len_gt_one():
...
def test_per_tile_physical_shape_rejects_hbm_split():
    """shared_hbm has no tile axis; list_len>1 is rejected loudly."""
    import pytest
...
def test_per_tile_physical_shape_rejects_versions_and_tiles_combo():
    """versions>1 combined with list_len>1 is not yet supported; rejected loudly."""
    import pytest
...
def test_per_tile_physical_shape_rejects_non_dividing_count():
    """list_len that does not divide the tile (middle) dim is rejected loudly."""
    import pytest

    from nkigym.ir.tree import Buffer

    buf = Buffer(name="s", shape=(2048, 512), dtype="bfloat16", location="sbuf", list_len=3)
