...
from nkigym.ir import GymProgram  # type: ignore[import]
from nkigym.utils.source import source_to_callable  # type: ignore[import]


def _shapes_to_kwargs(input_shapes: dict[str, tuple[int, ...]], dtype: type) -> dict[str, np.ndarray]:
    """Create zero-filled kernel kwargs from shapes and dtype."""
    return {name: np.zeros(shape, dtype=dtype) for name, shape in input_shapes.items()}
...
class TestSourceToProgram:
    ...
    def test_parse(
        ...
        actual = source_to_program(source, _shapes_to_kwargs(input_shapes, dtype))
    ...
    def test_dtype_propagated(self) -> None:
        ...
        kw = _shapes_to_kwargs({"a": (128, 128), "b": (128, 128)}, np.float64)
        program = source_to_program(source, kw)
    ...
    def test_error_unknown_op(self) -> None:
        ...
        with pytest.raises(KeyError, match="Unknown op"):
            source_to_program(source, _shapes_to_kwargs({"a": (128, 128)}, np.float32))
...
class TestRoundTrip:
    ...
    def test_source_round_trip(self, source: str, input_shapes: dict[str, tuple[int, ...]]) -> None:
        ...
        kernel_kwargs = _shapes_to_kwargs(input_shapes, np.float32)
        program = source_to_program(source, kernel_kwargs)
    ...
    def test_program_round_trip(self, program: GymProgram) -> None:
        ...
        result = source_to_program(source, program.kwargs)
        assert result == program

    def test_program_hashable(self) -> None:
        """Verify GymPrograms can be used as dict keys."""
        source = "def matmul(a, b):\n    return nkigym.nc_matmul(a, b)\n"
        p1 = source_to_program(source, _shapes_to_kwargs({"a": (128, 128), "b": (128, 128)}, np.float32))
        p2 = source_to_program(source, _shapes_to_kwargs({"a": (128, 256), "b": (128, 256)}, np.float32))
        d = {p1: "small", p2: "large"}
        assert len(d) == 2
        assert d[p1] == "small"
