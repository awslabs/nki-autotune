...
def nki_matmul_hoist_load_(lhsT, rhs, result):
    ...
    for m in nl.affine_range(M // TILE_M):
        ...
        lhsT_tiles = nl.ndarray(
            (K // TILE_K, nl.par_dim(TILE_K), TILE_N), dtype=lhsT.dtype, buffer=nl.sbuf
        )

        for k in nl.affine_range(K // TILE_K):
            # use `.p` for partition dimension and `.x` for the first free dimension
            lhsT_tiles[k, i_lhsT.p, i_lhsT.x] = nl.load(
                lhsT[k * TILE_K + i_lhsT.p, m * TILE_M + i_lhsT.x]
            )
        ...
        for n in nl.affine_range(N // TILE_N):
            ...
            rhs_tiles = nl.ndarray(
                (K // TILE_K, nl.par_dim(TILE_K), TILE_N),
                dtype=rhs.dtype,
                buffer=nl.sbuf,
            )
            for k in nl.affine_range(K // TILE_K):
                rhs_tiles[k, i_rhs.p, i_rhs.x] = nl.load(
                    rhs[k * TILE_K + i_rhs.p, n * TILE_N + i_rhs.x]
                )
            ...
            for k in nl.affine_range(K // TILE_K):
                # Accumulate partial-sums into PSUM
                res_psum[...] += nl.matmul(
                    lhsT_tiles[k, i_lhsT.p, i_lhsT.x],
...
def stack_allocated_fused_rms_norm_qkv(
    hidden, weights, norm_dtype=nl.float32, eps=1e-6
):
    """
    ...
    identity_tensor = nl.ndarray(
        (par_dim(pmax), 128), dtype=weights.dtype, buffer=nl.sbuf
    )
    ...
    for b in nl.affine_range(batch):
        weights_buffer = nl.ndarray(
            (M, par_dim(pmax), fmax), dtype=weights.dtype, buffer=nl.sbuf
        )
        ...
        for i in nl.affine_range(TILES_INT):
            ...
            in_bufs = nl.ndarray(
                (2, par_dim(pmax), dim), dtype=hidden.dtype, buffer=nl.sbuf
            )
            for i_interleave_grp in nl.affine_range(2):
                ...
                square_sum = nl.ndarray(
                    (par_dim(pmax), 1), dtype=norm_dtype, buffer=nl.sbuf
                )

                # Write the output of RMS and RMS^T (in-place) out to here
                out_tile = nl.ndarray(
                    (par_dim(pmax), dim), dtype=weights.dtype, buffer=nl.sbuf
                )

                # Store the final output tiles to here before sending back to DRAM
                output_sbuf = nl.ndarray(
                    (par_dim(pmax), fmax), dtype=weights.dtype, buffer=nl.sbuf
                )
                ...
                for m in nl.affine_range(NUM_TRANSP_TILES):
                    ...
                    transpose_res_psum = nl.ndarray(
                        (par_dim(pmax), 4 * pmax),
                        dtype=transpose_res_psum_dtype,
                        buffer=nl.psum,
                    )
                    ...
                    for j in nl.affine_range(4):
                        transpose_res_psum[i_lhs.p, j * pmax + i_lhs.x] = (
                            nisa.nc_matmul(
                    ...
                    out_tile[i_rhs.p, m * 4 * pmax + i_rhs.x] = nl.copy(
                        transpose_res_psum, dtype=hidden.dtype
                    )
...
def nki_rms_norm_(a_tensor, g_tensor, eps, PARTITION_DIM, FREE_DIM):
    ...
    ix = nl.arange(PARTITION_DIM)[:, None]
    iw = nl.arange(1)[:, None]
    ...
    for i in nl.affine_range(math.ceil(a_tensor.shape[1]/PARTITION_DIM)):
        # Load input data from external memory to on-chip memory
        #a_tile = nl.zeros([PARTITION_DIM, a_tensor.shape[2]], a_tensor.dtype)
        #a_tile[...] = nl.load(a_tensor[0, i * PARTITION_DIM + ix, iy], mask=(i * PARTITION_DIM + ix < num_rows))
        a_tile = nl.load(a_tensor[0, i * PARTITION_DIM + ix, iy], mask=(i * PARTITION_DIM + ix < num_rows))
        ...
        # num_rows_active = min(num_rows - i * PARTITION_DIM, PARTITION_DIM)
        g_bcast = g_tile.broadcast_to((PARTITION_DIM, g_tensor.shape[0]))

        # Multiply with the RMSNorm weight
        out_tile[...] = nl.multiply(out_tile, g_bcast, mask=(i * PARTITION_DIM + ix < num_rows))

        # store the addition results back to external memory (out_tensor)
        nl.store(out_tensor[0, i * PARTITION_DIM + ix, iy], value=out_tile, mask=(i * PARTITION_DIM + ix < num_rows))
