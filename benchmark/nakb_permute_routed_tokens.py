"""Measured NAKB ``permute_routed_tokens`` workload targets."""
...
def permute_routed_tokens_torch_ref(
    hidden_input: torch.Tensor, expert_index: torch.Tensor, expert_affinities_masked: torch.Tensor
) -> torch.Tensor:
    """Sort tokens by expert and pack the routed token data."""
    tokens, top_k = expert_index.shape
    _, experts = expert_affinities_masked.shape
    sorted_flat = argsort_unstable_torch_ref(expert_index.reshape(1, tokens * top_k), descending=False).flatten()
    token_indices = sorted_flat // top_k
    hidden_sorted = hidden_input[token_indices]
    sorted_expert_indices = expert_index.flatten()[sorted_flat]
    sorted_expert_offsets = torch.arange(0, tokens * experts, experts, dtype=torch.int32).repeat_interleave(top_k)[
        sorted_flat
    ]
    sorted_expert_indices_with_offset = sorted_expert_indices + sorted_expert_offsets
    affinities_sorted = expert_affinities_masked.flatten()[sorted_expert_indices_with_offset].unsqueeze(1)
    token_indices_bf16 = token_indices.reshape(tokens * top_k, 1).view(torch.bfloat16)
    return torch.cat([hidden_sorted, affinities_sorted, token_indices_bf16], dim=1)
...
_torch_ref_0 = TorchReference(
    permute_routed_tokens_torch_ref, ("hidden_input", "expert_index", "expert_affinities_masked")
)


WORKLOADS = (
    {
        "accuracy": AccuracySpec(atol=0.01, rtol=0.01, mode="global_max", output_indices=None, output_tolerances=()),
        "torch_ref": _torch_ref_0,
        "input_specs": {
            "hidden_input": ((16, 64), "bfloat16"),
            "expert_index": ((16, 2), "int32"),
            "expert_affinities_masked": ((16, 4), "bfloat16"),
        },
        "input_generator": _input_generator,
        "nakb_latency_ms": 0.026745791,
    },
    {
        "accuracy": AccuracySpec(atol=0.01, rtol=0.01, mode="global_max", output_indices=None, output_tolerances=()),
        "torch_ref": _torch_ref_0,
        "input_specs": {
            "hidden_input": ((64, 128), "bfloat16"),
            "expert_index": ((64, 2), "int32"),
            "expert_affinities_masked": ((64, 8), "bfloat16"),
        },
        "input_generator": _input_generator,
        "nakb_latency_ms": 0.038126607,
    },
)
