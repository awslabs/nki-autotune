...
def rewrite_one_candidate(
    ...
    return rewriter(context, graph, candidate)
...
def _rewrite_create(
    ...
    return reassemble(context, graph, absorbed, composite_op, inputs, outputs, candidate)
...
def _absorbed_ops(context: KernelContext, graph: KernelGraph, candidate: OnlineFusionCandidate) -> set[NKIOp]:
    ...
    """
    ops = all_ops(graph)
    forward = _bfs_reach(context, ops, [candidate.x_op], forward=True)
    backward = _bfs_reach(context, ops, list(candidate.accumulator_ops), forward=False)
    return forward & backward


def _bfs_reach(context: KernelContext, ops: list[NKIOp], starts: list[NKIOp], forward: bool) -> set[NKIOp]:
    """BFS following data-flow edges (forward / backward) over op instances."""
    adjacency: dict[NKIOp, list[NKIOp]] = {op: [] for op in ops}
    for producer in ops:
        for tname in context.op_outputs.get(producer, []):
            for consumer in ops:
                if tname in context.op_inputs.get(consumer, {}).values():
...
def _external_boundary(
    ...
    inputs = _role_order_inputs(context, external_in_names, candidate)
...
def _role_order_inputs(
    ...
    for op in candidate.accumulator_ops:
        if type(op).NAME == "nc_matmul":
            moving = context.op_inputs.get(op, {}).get("moving")
...
def _collect_external_outputs(
    ...
    for op in ops:
        ...
        for tname in context.op_outputs.get(op, []):
            if tname in seen:
                continue
            if is_externally_used(context, graph, tname, absorbed):
                names.append(tname)
                seen.add(tname)
    return names
...
def _build_composite_class(
    ...
    accumulator_specs = build_accumulator_specs(candidate, context, outputs)
