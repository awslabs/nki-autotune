...
from nkigym.ir.program_sharding import axis_loop_for_block, configured_program_shards, owning_block
from nkigym.ir.tree import BufferRegion, ISANode
from nkigym.ops.base import AxisRole, CopyContract, PeerExchangeContract, PointwiseContract
        ...
        if programs is not None:
            for leaf_nid in ir.dependency.graph.nodes:
                if _is_replicated_store(ir, leaf_nid):
                    ...
                    axes = tuple(dict.fromkeys(item.axis for item in block.iter_vars if item.role == AxisRole.PARALLEL))
                    for axis in axes:
...
def _is_replicated_store(ir: KernelIR, leaf_nid: int) -> bool:
    ...
    if not isinstance(store, ISANode):
        return False
    ...
    source = store.operand_bindings.get(contract.input_operand) if isinstance(contract, CopyContract) else None
    if (
        not isinstance(contract, CopyContract)
        ...
        return False
    combined = _copy_origin(ir, leaf_nid, source)
    if combined is None:
        return False
    combine_nid, combined_region = combined
    combine = ir.tree.isa(combine_nid)
    combine_contract = combine.op_cls.algebraic_contract(combine.kwargs)
    if (
        not isinstance(combine_contract, PointwiseContract)
        or combine_contract.operator not in {"add", "maximum", "multiply"}
        or len(combine_contract.input_operands) != 2
        or not set(combine_contract.input_operands) <= combine.operand_bindings.keys()
        or combine.operand_bindings.get(combine_contract.output_operand) != combined_region
    ):
        return False
    values = tuple(
        _program_value(ir, combine_nid, combine.operand_bindings[slot]) for slot in combine_contract.input_operands
    )
...
def _program_value(
    ...
    if not isinstance(contract, PeerExchangeContract):
        return False, origin
    source = producer.operand_bindings.get(contract.input_operand)
    destination = producer.operand_bindings.get(contract.output_operand)
    routed = destination == produced_region
...
def _copy_origin(ir: KernelIR, consumer_nid: int, region: BufferRegion) -> tuple[int, BufferRegion] | None:
    ...
    while True:
        ...
        if not isinstance(contract, CopyContract):
            written = tuple(
                item for item in ir.dependency.info(producer_nid).write_regions if item.tensor == current_region.tensor
            )
