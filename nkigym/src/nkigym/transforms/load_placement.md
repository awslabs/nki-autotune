...
See `kernel_ir/load_placement.md` for the IR field (`tensor_placements`), the three-tier semantics, the joint feasibility rule against `group_dim_orders` (per-group), and the `num_tiles` shape formula. This doc only describes the transform that mutates `tensor_placements`.

## What It Does

For each `(tensor, dim)` entry in `ir.tensor_placements`, try promoting the tier one step — `per_tile → per_block → full` — if the promotion (a) strictly grows the buffer on that dim and (b) remains feasible against the current `group_dim_orders` (feasibility is checked in each group's nest where the tensor appears). Each feasible promotion is one candidate.
...
After applying the new tier, run the feasibility rule from `kernel_ir/load_placement.md`: build the tensor's $A$ (above-alloc) and $B$ (below-alloc) loop sets, then verify every $A$ loop precedes every $B$ loop under the phase-grouped emission of each relevant group's `dim_order`. Reject if not.
...
- **Loop reordering** can enable previously-infeasible placements. Candidate generation runs against the current `group_dim_orders`; if a desired hoist is blocked, the search may reach it later via a reorder-then-hoist sequence.
...
Matmul `lhs_T(K=d0, M=d1) × rhs(K=d0, N=d2) → result(d1, d2)`. Single fusion group with `group_dim_orders = [["d0", "d1", "d2"]]` (d0 is the blocking axis — matmul's K — sorted outermost per the group's dim-order convention).

Initial `tensor_placements` (all `per_tile`). Candidate for `("rhs", "d0")` → `full`:
- Growth: `tpb × num_blocks = 1 × 16 = 16 > 1`. OK.
- Feasibility: rhs is on `(d0, d2)`. Under the new assignment, d2 remains `per_tile` so its loops stay in $A$; d0 becomes `full` so its loops move to $B$. Result: $A$ = {block_d1, block_d2, tile_d1, tile_d2}, $B$ = {block_d0, tile_d0}. Under the group's phase-grouped emission, non-blocking loops precede blocking loops — feasible. Allocation rises from the innermost blocking body to the innermost non-blocking body.

The buffer grows from `(128, 1, 1, 128)` to `(128, 16, 1, 128)` on d0. DMA fires once per non-blocking tile instead of per-blocking-iteration — 16× fewer loads.
