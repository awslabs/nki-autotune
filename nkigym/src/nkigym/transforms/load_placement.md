...
Storing tiers instead of raw counts keeps the representation valid when $\texttt{tpb\_hbm}$ or $\texttt{tpb\_psum}$ changes — "per_block" always means "between block and psum-batch loops", "per_psum_batch" always means "between psum-batch and tile loops", and derived buffer sizes adjust automatically. With raw counts, a stored value can become orphaned (matching no tier) after a parameter change, leaving the renderer unable to determine the pre-load loop position.
...
Tier distinctness is checked via derived sub-tile counts — each tier is only offered when its count is strictly between the current count and the next higher tier's count (avoids duplicates). Per-tile is offered only when distinct from both single ($\texttt{interleave} > 1$) and per-psum-batch ($\texttt{tpb\_psum} > 1$). Per-psum-batch is offered only when distinct from both per-tile ($\texttt{tpb\_psum} > 1$) and per-block ($\texttt{tpb\_hbm} > \texttt{tpb\_psum}$).
...
2. **DMA moves to a pre-load loop.** Instead of loading one sub-tile per compute iteration, the renderer emits a dedicated loop at the target position (between tile and ig for per-tile, between psum-batch and tile for per-psum-batch, between block and psum-batch for per-block, above block for full) that fills all buffer slots before the compute loop begins.
...
for i_block_d0 in nl.affine_range(16):
    for i_psum_batch_d0 in nl.affine_range(1):
        for i_tile_d0 in nl.affine_range(1):
            for i_block_d4 in nl.affine_range(1):
                for i_psum_batch_d4 in nl.affine_range(1):
                        ...
                        for i_interleave_group_d0 in nl.affine_range(1):
                            for i_interleave_group_d4 in nl.affine_range(1):
                                ...
                                for i_block_d2 in nl.affine_range(4):
                                    for i_psum_batch_d2 in nl.affine_range(1):
...
|---|---|---|---|
| K | per-block on $d_2$ | $\texttt{tpb\_hbm}(d_2) \times \texttt{interleave}(d_2)$ sub-tiles | `bufs.k_sb` loaded per-section |
| V | per-block on $d_2$ | $\texttt{tpb\_hbm}(d_2) \times \texttt{interleave}(d_2)$ sub-tiles | `bufs.v_sb` loaded per-section |
| Q | single on $d_0$ (+ multi-buffer D=2) | 1 sub-tile (double-buffered separately) | `bufs.q_sb` loaded per-group |

K and V at per-block on $d_2$ sit between $d_2$'s block (section) and psum-batch loops. With $d_0$ (irrelevant to K/V) interleaved between them, K/V are loaded once per section and reused across all Q groups. Q stays at single — in the fused group, scheduling (§5.1) places Q's ops outside $d_2$'s loops since Q doesn't depend on $d_2$.
