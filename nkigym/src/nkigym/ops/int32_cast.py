"""Cast one local tile to int32 with ``nisa.activation``."""
...
class NKIInt32Cast(NKIOp):
    """Copy one tile into an int32 destination."""
    ...
    def __init__(self) -> None:
        """Configure the native activation as an explicit int32 copy."""
    ...
    def _check_roles(self, **kwargs: Any) -> None:
        ...
        if role is not None and role not in {"sbuf", "psum"}:
            raise TypeError(f"NKIInt32Cast(data=<role={role}>) expects sbuf or psum")

    def _run(self, **kwargs: Any) -> np.ndarray:
        """Return the source values represented as int32."""
        return np.asarray(kwargs["data"], dtype=np.int32)


__all__ = ["NKIInt32Cast"]
