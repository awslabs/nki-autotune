...
class NKIHBMScalarRowSlice(NKIOp):
    """Load one matrix selected by a scalar SBUF index."""
    ...
    CODEGEN_ONLY_KWARGS: ClassVar[frozenset[str]] = frozenset({"rows", "width"})
    ...
    def _check_roles(self, **kwargs: Any) -> None:
        ...
        if source.ndim != 2 or source.shape[1] != rows * width or indices.size != 1:
    ...
    def _run(self, **kwargs: Any) -> np.ndarray:
        ...
        index = int(np.asarray(kwargs["indices"]).reshape(-1)[0])
        if index < 0 or index >= source.shape[0]:
            raise ValueError("scalar HBM row slice index exceeds the expert extent")
        return source[index].reshape(int(kwargs["rows"]), int(kwargs["width"])).copy()
