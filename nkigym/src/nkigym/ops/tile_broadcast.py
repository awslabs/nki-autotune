...
class NKITileBroadcast(NKIOp):
    ...
    OPERAND_AXES: ClassVar[dict[str, tuple[str, ...]]] = {"src": ("M", "G", "N"), "dst": ("M", "C", "T", "G", "N")}
    ...
    MAX_TILE_SIZE: ClassVar[dict[str, int | None]] = {"M": None, "C": 1, "T": 1, "G": None, "N": None}
    NON_TILABLE_AXES: ClassVar[frozenset[str]] = frozenset("MGN")
    ...
    def __init__(self, chunks: int, tiles: int, groups: int, queries: int) -> None:
        ...
        super().__init__(chunks=chunks, tiles=tiles, groups=groups, queries=queries)

    def _check_roles(self, **kwargs: Any) -> None:
        """Require one SBUF source."""
        if (role := _operand_role(kwargs["src"])) is not None and role != "sbuf":
            raise TypeError(f"NKITileBroadcast(src=<role={role}>) expects sbuf")
    ...
    def _run(self, **kwargs: Any) -> np.ndarray:
        ...
        data = np.asarray(kwargs["src"]).reshape(-1, g, n)
