"""Element-wise binary addition op definition for schedule-based rendering."""
...
class NKIAdd(NKIOp):
    """Element-wise add: ``nisa.tensor_tensor(dst=..., data1=..., data2=..., op=nl.add)``.

    Attributes:
        op_name: Registry key ``"add"``.
    ...
    def render_post_compute(
        ...
        """Render element-wise add as a post-compute op.

        Args:
            dst_expr: Destination SBUF expression.
            operand_exprs: Must contain ``"data1"`` and ``"data2"`` keys.
            config_kwargs: Unused for add.

        Returns:
            NKI tensor_tensor source line with ``op=nl.add``.
        """
        return f"nisa.tensor_tensor(dst={dst_expr}, data1={operand_exprs['data1']}, data2={operand_exprs['data2']}, op=nl.add)"
