...
a graph node) — the producer writes to PSUM, the renderer
...
def dma_load_line(ir: KernelIR, op: NKIOp, group_idx: int, depth: int) -> str:
    ...
    sbuf_name = outputs[0]
    tinfo = context.logical_tensors[sbuf_name]
...
def ptile_loop_dims(ir: KernelIR, op: NKIOp) -> list[tuple[str, int]]:
    ...
    seen: set[str] = set()
    tensor_names = [*context.op_inputs.get(op, {}).values(), *context.op_outputs.get(op, [])]
