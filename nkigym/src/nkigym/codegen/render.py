...
def render_forest(w: _Writer, op_graph: OpGraph, forest: LoopForest) -> None:
    ...
    counts so body emitters can build correct slot expressions via
    :func:`_slot_expr`.
    """
    path_ordinals: dict[str, int] = {}
    path_trips: dict[str, list[int]] = {}
...
def _body_load(w, op_graph, op, path_ordinals, path_trips) -> None:
    ...
    dst_expr = _sbuf_tile_slice(_sbuf_name(dst_name), dst_tensor.dim_ids, p_tile, f_tile, path_ordinals, path_trips)
    src_expr = _hbm_tile_slice(src_name, src_tensor.dim_ids, p_tile, f_tile, path_ordinals, path_trips)
...
def _body_store(w, op_graph, op, path_ordinals, path_trips) -> None:
    ...
    p_tile = op_graph.dims[p_axis].tile_size
    f_tile = op_graph.dims[f_axis].tile_size if f_axis is not None else 1
    dst_expr = _hbm_tile_slice(_hbm_name(dst_name), dst_tensor.dim_ids, p_tile, f_tile, path_ordinals, path_trips)
    src_expr = _sbuf_tile_slice(_sbuf_name(src_name), src_tensor.dim_ids, p_tile, f_tile, path_ordinals, path_trips)
    w.line(f"nisa.dma_copy(dst={dst_expr}, src={src_expr})")
...
def _body_activation(w, op_graph, op, path_ordinals, path_trips) -> None:
    ...
    dst_expr = _sbuf_tile_slice(_sbuf_name(dst_name), dst.dim_ids, p_tile, f_tile, path_ordinals, path_trips)
    src_expr = _sbuf_tile_slice(_sbuf_name(src_name), src.dim_ids, p_tile, f_tile, path_ordinals, path_trips)
...
def _body_tensor_scalar(w, op_graph, op, path_ordinals, path_trips) -> None:
    ...
    dst_expr = _sbuf_tile_slice(_sbuf_name(dst_name), dst.dim_ids, p_tile, f_tile, path_ordinals, path_trips)
    data_expr = _sbuf_tile_slice(_sbuf_name(data_name), data.dim_ids, p_tile, f_tile, path_ordinals, path_trips)
    op0_expr = _sbuf_tile_slice(_sbuf_name(op0_name), op0.dim_ids, p_tile, 1, path_ordinals, path_trips)
...
def _body_transpose(w, op_graph, op, path_ordinals, path_trips) -> None:
    ...
    dst_expr = _swapped_dst_tile_slice(dst_name, src_p_axis, src_f_axis, p_tile, path_ordinals, path_trips)
...
def _body_matmul_compute(w, op_graph, op, path_ordinals, path_trips) -> None:
    ...
    stat_expr = _sbuf_tile_slice(_sbuf_name(stat_name), stat.dim_ids, p_tile_K, p_tile_M, path_ordinals, path_trips)
    mov_expr = _sbuf_tile_slice(_sbuf_name(mov_name), mov.dim_ids, p_tile_K, f_tile_N, path_ordinals, path_trips)
...
def _body_matmul_drain(w, op_graph, op, path_ordinals, path_trips) -> None:
    ...
    p_tile_M = op_graph.dims[m_dim].tile_size
    f_tile_N = op_graph.dims[n_dim].tile_size
    out_expr = _sbuf_tile_slice(_sbuf_name(out_name), out.dim_ids, p_tile_M, f_tile_N, path_ordinals, path_trips)
