...
def f_nkigym(lhs, rhs):
    ...
    # LoopNode(dim_id="d1", trip=16, role=PARALLEL, name="i_d1_0")  path=(0,)
    for i_d1_0 in range(16):
        # LoopNode(dim_id="d1", trip=1, role=PARALLEL, name="i_d1_1")  path=(0, 0)
        for i_d1_1 in range(1):
            # LoopNode(dim_id="d3", trip=4, role=PARALLEL, name="i_d3_0")  path=(0, 0, 0)
            for i_d3_0 in range(4):
                # LoopNode(dim_id="d3", trip=1, role=PARALLEL, name="i_d3_1")  path=(0, 0, 0, 0)
                for i_d3_1 in range(1):
                    # BodyLeaf(op_idx=1, phase="main")  path=(0, 0, 0, 0, 0)
                    nisa.dma_copy(
    ...
    # LoopNode(dim_id="d0", trip=16, role=PARALLEL, name="i_d0_0")  path=(1,)
    for i_d0_0 in range(16):
        # LoopNode(dim_id="d0", trip=1, role=PARALLEL, name="i_d0_1")  path=(1, 0)
        for i_d0_1 in range(1):
            # LoopNode(dim_id="d1", trip=16, role=PARALLEL, name="i_d1_0")  path=(1, 0, 0)
            for i_d1_0 in range(16):
                # LoopNode(dim_id="d1", trip=1, role=PARALLEL, name="i_d1_1")  path=(1, 0, 0, 0)
                for i_d1_1 in range(1):
                    # BodyLeaf(op_idx=0, phase="main")  path=(1, 0, 0, 0, 0)
                    nisa.dma_copy(
        ...
        # LoopNode(dim_id="d0", trip=1, role=PARALLEL, name="i_d0_1")  path=(1, 1)
        for i_d0_1 in range(1):
            # LoopNode(dim_id="d1", trip=16, role=ACCUMULATION, name="i_d1_0")  path=(1, 1, 0)
            for i_d1_0 in range(16):
                # LoopNode(dim_id="d1", trip=1, role=ACCUMULATION, name="i_d1_1")  path=(1, 1, 0, 0)
                for i_d1_1 in range(1):
                    # BodyLeaf(op_idx=2, phase="reduce_step")  path=(1, 1, 0, 0, 0)
            ...
            # BodyLeaf(op_idx=2, phase="reduce_close")  path=(1, 1, 1)
    ...
    for i_d0_0 in range(16):
        ...
        for i_d0_1 in range(1):
            # BodyLeaf(op_idx=3, phase="main")  path=(2, 0, 0)
            nisa.activation(
        ...
        # LoopNode(dim_id="d0", trip=1, role=PARALLEL, name="i_d0_1")  path=(2, 1)
        for i_d0_1 in range(1):
            # LoopNode(dim_id="d1", trip=16, role=PARALLEL, name="i_d1_0")  path=(2, 1, 0)
            for i_d1_0 in range(16):
                # LoopNode(dim_id="d1", trip=1, role=PARALLEL, name="i_d1_1")  path=(2, 1, 0, 0)
                for i_d1_1 in range(1):
                    # BodyLeaf(op_idx=4, phase="main")  path=(2, 1, 0, 0, 0)
                    nisa.tensor_scalar(
                        dst=sbuf_normed[
    ...
    # LoopNode(dim_id="d1", trip=16, role=PARALLEL, name="i_d1_0")  path=(3,)
    for i_d1_0 in range(16):
        # LoopNode(dim_id="d1", trip=1, role=PARALLEL, name="i_d1_1")  path=(3, 0)
        for i_d1_1 in range(1):
            # LoopNode(dim_id="d0", trip=16, role=PARALLEL, name="i_d0_0")  path=(3, 0, 0)
            for i_d0_0 in range(16):
                # LoopNode(dim_id="d0", trip=1, role=PARALLEL, name="i_d0_1")  path=(3, 0, 0, 0)
                for i_d0_1 in range(1):
                    # BodyLeaf(op_idx=5, phase="main")  path=(3, 0, 0, 0, 0)
    ...
    # LoopNode(dim_id="d0", trip=16, role=PARALLEL, name="i_d0_0")  path=(4,)
    for i_d0_0 in range(16):
        # LoopNode(dim_id="d0", trip=1, role=PARALLEL, name="i_d0_1")  path=(4, 0)
        for i_d0_1 in range(1):
            # LoopNode(dim_id="d3", trip=4, role=PARALLEL, name="i_d3_0")  path=(4, 0, 0)
            for i_d3_0 in range(4):
                # LoopNode(dim_id="d3", trip=1, role=PARALLEL, name="i_d3_1")  path=(4, 0, 0, 0)
                for i_d3_1 in range(1):
                    # BodyLeaf(op_idx=6, phase="psum_init")  path=(4, 0, 0, 0, 0)
                    psum_tile = nl.ndarray((128, 512), dtype=nl.float32, buffer=nl.psum)
                    nisa.memset(psum_tile[0:128, 0:512], value=0.0)
                    # LoopNode(dim_id="d1", trip=16, role=ACCUMULATION, name="i_d1_0")  path=(4, 0, 0, 0, 1)
                    for i_d1_0 in range(16):
                        # LoopNode(dim_id="d1", trip=1, role=ACCUMULATION, name="i_d1_1")  path=(4, 0, 0, 0, 1, 0)
                        for i_d1_1 in range(1):
                            # BodyLeaf(op_idx=6, phase="compute")  path=(4, 0, 0, 0, 1, 0, 0)
                            nisa.nc_matmul(
                    ...
                    # BodyLeaf(op_idx=6, phase="drain")  path=(4, 0, 0, 0, 2)
                    nisa.tensor_copy(
                        sbuf_matmul_out[
    ...
    for i_d0_0 in range(16):
        ...
        for i_d0_1 in range(1):
            ...
            for i_d3_0 in range(4):
                ...
                for i_d3_1 in range(1):
                    # BodyLeaf(op_idx=7, phase="main")  path=(5, 0, 0, 0, 0)
                    nisa.dma_copy(
                        dst=hbm_out[
                            (i_d0_0 + i_d0_1) * 128 : (i_d0_0 + i_d0_1) * 128 + 128,
                            (i_d3_0 + i_d3_1) * 512 : (i_d3_0 + i_d3_1) * 512 + 512,
