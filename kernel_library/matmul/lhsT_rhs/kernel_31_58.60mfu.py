...
def matmul_lhsT_rhs_nkigym(lhs_T, rhs):
    ...
    for i_block_d0 in range(2):
        for i_ltile_d0 in range(8):
            for i_block_d1 in range(2):
                for i_ltile_d1 in range(8):
                    pass

    # Group 1: dma_load [dims: d2, d0]
    ...
    # Group 2: nc_matmul [dims: d1, d2, d0]
                    ...
                    for i_block_d0 in range(2):
                        for i_ltile_d0 in range(8):
                            nisa.nc_matmul(
                                ...
                                stationary=sbuf_lhs_T[i_block_d0 * 8 + i_ltile_d0][i_block_d1 * 8 + i_ltile_d1][
                                    0:128, 0:128
                                ],
                                moving=sbuf_rhs[i_block_d0 * 8 + i_ltile_d0][i_ltile_d2][0:128, 0:512],
                            )
                    stage_block(sbuf_output, psum_output, i_block_d1 * 8 + i_ltile_d1, 1, i_ltile_d2, 1)

    # Group 3: dma_store [dims: d1, d2]
