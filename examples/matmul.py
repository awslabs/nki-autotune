...
Four input-layout variants:
  ...
  * ``lhs_T @ rhs_T`` -- lhs_T(K, M), rhs_T(N, K) -> transpose rhs_T, then nc_matmul
...
def matmul_lhsT_rhsT_nkigym(lhs_T: np.ndarray, rhs_T: np.ndarray) -> np.ndarray:
    """Matrix multiply using nkigym logical ops: lhs_T.T @ rhs_T.T."""
    rhs = NKITranspose()(data=rhs_T)
    output = NKIMatmul()(stationary=lhs_T, moving=rhs)
    return output
...
if __name__ == "__main__":
    ...
    remote_search(
        func=matmul_lhsT_rhsT_nkigym,
        input_specs={"lhs_T": ((K, M), "bfloat16"), "rhs_T": ((N, K), "bfloat16")},
        hosts=HOSTS,
        cache_dir=str(CACHE_ROOT / "lhsT_rhsT"),
        num_variants=NUM_VARIANTS,
        atol=ATOL,
        rtol=RTOL,
        seed=0,
