...
def get_autotune_jobs(M: int, N: int, K: int) -> ProfileJobs:
    ...
    for NUM_BLOCK_M, NUM_BLOCK_N, NUM_BLOCK_K, BUFFER_M, BUFFER_N, BUFFER_K, loop_order in params:
        ...
        jobs.add_job(
            kernel=matmul_main, kernel_args=(lhsT, rhs), pruning_func=GEMMCompatibility(transposed_lhs=True), **config
        )
    return jobs
...
def profile(workload_name: str, M: int, N: int, K: int):
    ...
    rhs = np.zeros((K, N), dtype=bfloat16)
    jobs = ProfileJobs()
    jobs.add_job(kernel=matmul_xt_op, kernel_args=(lhsT, rhs), pruning_func=GEMMCompatibility(transposed_lhs=True))
