...
Information from IR:
```
producer of sbuf_output = [3] NKIMatmul, dim_role: {'K'=d1: ACCUMULATION, 'M'=d0: PARALLEL, 'N'=d2: PARALLEL}
```
    ...
    for i_block_d2 in range(4):
        ...
        for i_block_d0 in range(2):
            for i_block_d1 in range(4):
                # sbuf_lhs_T consumed here because sbuf_lhs_T = INNER (depends on d1, d0)
                cur_sbuf_lhs_T = sbuf_lhs_T[i_block_d1 % 2][i_block_d0 % 4]
                load_block(cur_sbuf_lhs_T, lhs[i_block_d0 * 1024 : i_block_d0 * 1024 + 1024, i_block_d1 * 512 : i_block_d1 * 512 + 512], transpose=True)
```
...
Information from IR:
```
producer of sbuf_output = [2] NKIMatmul, dim_role: {'K'=d1: ACCUMULATION, 'M'=d0: PARALLEL, 'N'=d2: PARALLEL}
```
Derive `store_block` placement:
```python
"""Derived from IR"""
