# Logical Function
```python
def rmsnorm_matmul_nkigym(x, gamma, W):
    """nkigym math function for ``rmsnorm(x) * gamma @ W``.

    rmsnorm(x) = x * rsqrt(mean(x², axis=K) + eps)
    output    = (rmsnorm(x) * gamma) @ W

    ``NKIMatmul.stationary`` expects ``(K, M)`` layout, so an inline
    ``NKITranspose`` converts ``x_gamma(M, K)`` → ``x_T(K, M)`` first.
    """
    ...
    x_gamma = NKITensorScalar(op='multiply')(data=x_rms, operand0=gamma)
    x_T     = NKITranspose()(data=x_gamma)
...
# KernelIR
```bash
KernelIR(func=rmsnorm_matmul_nkigym, params=['x', 'gamma', 'W'], return=output)
    ...
    input_hbm_tensors:
        ...
        hbm_gamma: shape=(2048,),      dims=('d1',),       dtype=bfloat16
    ...
    physical_buffers:
        ...
        sbuf_gamma:   tile=(128,),     dims=('d1',),       dtype=bfloat16
        ...
        sbuf_x_gamma: tile=(128, 128), dims=('d0', 'd1'), dtype=bfloat16
    ...
    operators:
        ...
        [1] NKILoad:
            data=gamma, outputs=[sbuf_gamma], dim_map={'P': 'd1'}
        [2] NKILoad:
            data=W, outputs=[sbuf_W], dim_map={'P': 'd1', 'F':'d2'}
        [3] NKIActivationReduce:
        ...
        [5] NKITensorScalar:
            data=sbuf_x_rms, operand0=sbuf_gamma, outputs=[sbuf_x_gamma], op='multiply', dim_map={'P': 'd0', 'F':'d1'}
        [6] NKITranspose:
            data=sbuf_x_gamma, outputs=[sbuf_x_T], dim_map={'P': 'd0', 'F':'d1'}
        [7] NKIMatmul:
    ...
    edges: (0, 3), (0, 4), (1, 5), (2, 7), (3, 4), (4, 5), (5, 6), (6, 7), (7, 8)
    loop_order: ['d2', 'd0', 'd1']
    ltiles/block:
        d0: 8
        d1: 4
    ...
    num_buffers:
        ...
        sbuf_gamma:   {num_p_buffers: 2,    num_f_buffers: None}  # rotate on d1 only
        ...
        sbuf_x_gamma: {num_p_buffers: 2,    num_f_buffers: 4}     # rotate on d0, d1
    ...
    emission_depth:
        ...
        sbuf_gamma:   0    # gamma is d1-only, loaded once at kernel top
        ...
        sbuf_x_gamma: 1    # inside loop_order[0] = i_block_d2
...
sbuf_gamma: tile=(128,), dims=('d1',), dtype=bfloat16
sbuf_gamma = INNER
sbuf_gamma:  {num_p_buffers: 2, num_f_buffers: None}
sbuf_gamma:  0
```
Derive `sbuf_gamma` buffer allocation:
```python
"""Directly read from IR"""
p_tile_size = 128
f_tile_size = 1   # gamma is 1D (partition-only); free-axis is phys=1
loc=nl.sbuf
dtype=nl.bfloat16
num_p_buffers = 2
num_f_buffers = None
...
"""Derived from IR"""
num_p_tiles = d1_num_ltile = 16 # Because outside of d1 (emission depth 0, full d1 axis resident)
num_f_tiles = 1                 # gamma has no free axis
```
Accumulated code generation:
```python
@nki.jit
def rmsnorm_matmul_nkigym(x, gamma, W):
    ...
    # sbuf_gamma declared here because sbuf_gamma emission depth = 0
    sbuf_gamma = allocate_buffers(p_tile_size = 128, num_p_tiles=16, f_tile_size = 1, num_f_tiles=1, loc=nl.sbuf, dtype=nl.bfloat16, num_p_buffers = 2, num_f_buffers = None)

    # gamma is d1-only; load once for every d1-ltile at kernel top
    for i_ltile_d1 in range(16):
        cur_sbuf_gamma = sbuf_gamma[i_ltile_d1 % 2]
        load_block(cur_sbuf_gamma, gamma[i_ltile_d1 * 128 : i_ltile_d1 * 128 + 128], transpose=False)
...
sbuf_rms_inv:  1
producer of sbuf_rms_inv reduces d1 (blocking_dims={d1}); d1: role=ACCUMULATION
```
...
def rmsnorm_matmul_nkigym(x, gamma, W):
    ...
    for i_block_d2 in range(4):
        ...
        for i_block_d0 in range(2):
            ...
            # producer has blocking_dims={d1} — memset and reduce across i_block_d1
            ...
            for i_block_d1 in range(4):
                ...
                # op=square, reduce_op=add → accumulates sum of squares into cur_sbuf_rms_inv
                activation_reduce_block(cur_sbuf_rms_inv, cur_sbuf_x, op='square', reduce_op='add', accumulate=True)
            # post_op=rsqrt applied once i_block_d1 closes; scale=1/K folded into rsqrt input, bias=eps
    ...
    for i_block_d2 in range(4):
        ...
        for i_block_d0 in range(2):
            ...
            for i_block_d1 in range(4):
                ...
                cur_sbuf_x     = sbuf_x[i_block_d0 % 2][i_block_d1 % 4]
                # tensor_scalar_block broadcasts cur_sbuf_rms_inv (per-row scalar) across F axis of cur_sbuf_x
...
sbuf_x_gamma: tile=(128, 128), dims=('d0', 'd1'), dtype=bfloat16
sbuf_x_gamma = INNER
sbuf_x_gamma:  {num_p_buffers: 2, num_f_buffers: 4}
sbuf_x_gamma:  1
```
Derive `sbuf_x_gamma` buffer allocation:
```python
"""Directly read from IR"""
p_tile_size = 128
f_tile_size = 128
...
def rmsnorm_matmul_nkigym(x, gamma, W):
    ...
    for i_block_d2 in range(4):
        ...
        sbuf_rms_inv = allocate_buffers(p_tile_size = 128, num_p_tiles=8,  f_tile_size = 1,   num_f_tiles=1, loc=nl.sbuf, dtype=nl.float32,  num_p_buffers = 2, num_f_buffers = None)
        sbuf_x_rms   = allocate_buffers(p_tile_size = 128, num_p_tiles=8,  f_tile_size = 128, num_f_tiles=4, loc=nl.sbuf, dtype=nl.bfloat16, num_p_buffers = 2, num_f_buffers = 4)
        # sbuf_x_gamma declared here because sbuf_x_gamma emission depth = 1
        sbuf_x_gamma = allocate_buffers(p_tile_size = 128, num_p_tiles=8,  f_tile_size = 128, num_f_tiles=4, loc=nl.sbuf, dtype=nl.bfloat16, num_p_buffers = 2, num_f_buffers = 4)
        for i_block_d0 in range(2):
            cur_sbuf_rms_inv = sbuf_rms_inv[i_block_d0 % 2]
            memset_buffers(cur_sbuf_rms_inv, 0.0)
            for i_block_d1 in range(4):
            ...
            for i_block_d1 in range(4):
                ...
                cur_sbuf_x       = sbuf_x[i_block_d0 % 2][i_block_d1 % 4]
                tensor_scalar_block(cur_sbuf_x_rms, cur_sbuf_x, cur_sbuf_rms_inv, op='multiply')
                # sbuf_x_gamma consumed here because sbuf_x_gamma = INNER (depends on d0, d1)
                # tensor_scalar_block broadcasts cur_sbuf_gamma (per-col scalar) across P axis of cur_sbuf_x_rms
                cur_sbuf_x_gamma = sbuf_x_gamma[i_block_d0 % 2][i_block_d1 % 4]
                cur_sbuf_gamma_d1 = [sbuf_gamma[(i_block_d1 * 4 + k) % 2] for k in range(4)]  # 4 d1-ltiles per d1-block
                tensor_scalar_block(cur_sbuf_x_gamma, cur_sbuf_x_rms, cur_sbuf_gamma_d1, op='multiply')
```
    ...
    for i_block_d2 in range(4):
        ...
        for i_block_d0 in range(2):
            ...
            for i_block_d1 in range(4):
                ...
                tensor_scalar_block(cur_sbuf_x_rms, cur_sbuf_x, cur_sbuf_rms_inv, op='multiply')
                cur_sbuf_x_gamma = sbuf_x_gamma[i_block_d0 % 2][i_block_d1 % 4]
                cur_sbuf_gamma_d1 = [sbuf_gamma[(i_block_d1 * 4 + k) % 2] for k in range(4)]
                tensor_scalar_block(cur_sbuf_x_gamma, cur_sbuf_x_rms, cur_sbuf_gamma_d1, op='multiply')
                ...
                transpose_block(cur_sbuf_x_T, cur_sbuf_x_gamma)
```
...
Information from IR:
```
producer of sbuf_output = [7] NKIMatmul, blocking_dims = {d1}
